...
public class PEmbroiderGraphics {
	 ...
	 *
	 * @param _app the running PApplet instance: in Processing, just pass the
	 *             keyword 'this'
	 * @param w    width
	 * @param h    height
	 */
	...
	/**
	 * Set the output file path. The filename should include the file type
	 * extension. Supported embroidery formats are: .DST, .EXP, .JEF, .PEC, .PES,
	 * .VP3, and .XXX. Additionally supported formats are: .PDF, .SVG, .TSV, and
	 * .GCODE.
	 * 
	 * @param _path output file path. The format will be automatically inferred from
	 *              the extension.
	 */
	 ...
	 * 
	 * @param j Same as Processing strokeCap, this can be ROUND, SQUARE, PROJECT
	 *          etc.
	 * @see strokejoin
	 */
	 ...
	 * which parameters given to ellipse() are intepreted. also governs circle()
	 * 
	 * @param j Same as Processing ellipseMode, this can be RADIUS, CENTER, CORNER,
	 *          CORNERS etc.
	 * @see rectMode
	 */
	 ...
	 * 
	 * @param j Same as Processing rectMode, this can be RADIUS, CENTER, CORNER,
	 *          CORNERS etc.
	 * @see ellipseMode
	 */
	 ...
	 * 
	 * @param mode This can be one of: PARALLEL, CROSS, CONCENTRIC, SPIRAL, PERLIN,
	 *             VECFIELD, DRUNK
	 */
	 ...
	 * 
	 * @param mode    This can be either PERPENDICULAR or TANGENT
	 * @param tanMode This can be one of COUNT (stroke weight used as line count),
	 *                WEIGHT (honour stroke weight setting over spacing) or SPACING
	 *                (honour spacing over stroke weight)
	 */
	 ...
	 * 
	 * @param ang1 the angle from +x in radians (for parallel hatches and the first
	 *             direction of cross hatching)
	 * @param ang2 the angle from +x in radians (for second direction of cross
	 *             hatching)
	 * @see hatchAngle
	 * @see hatchAnglesDeg
	 */
	 ...
	 * Change angles of parallel and cross hatching lines
	 * 
	 * @param ang1 the angle from +x in degrees (for parallel hatches and the first
	 *             direction of cross hatching)
	 * @param ang2 the angle from +x in degrees (for second direction of cross
	 *             hatching)
	 * @see hatchAngles
	 * @see hatchAngleDeg
	 */
	 ...
	 * 
	 * @param mode one of ADAPTIVE (use most appropriate method for each situation
	 *             according to Lingdong) FORCE_VECTOR (uses vector math whenever
	 *             possible) FORCE_RASTER (first render shapes as raster and
	 *             re-extract the structures, generally more robust)
	 */
	 ...
	 * 
	 * @param vf a vector field defination -- simple, just a class with a get(x,y)
	 *           method
	 */
	...
	/**
	 * Set the desirable stitch length. Stitches will try their best to be around
	 * this length, but actual length will vary slightly for best result
	 * 
	...
	/**
	 * Set the minimum stitch length. Drawings with higher precision than this will
	 * be resampled down to have at least this stitch length
	 * 
	 ...
	 * @param rn  resample noise -- to avoid alignment patterns; must be in the
	 *            range [0...1]
	 */
	...
	/**
	 * Turn resampling on and off. For embroidery machines, you might want it; for
	 * plotters you probably won't need it.
	 * 
	 ...
	 * 
	 * @param mode This can be CLOCKWISE or COUNTERCLOCKWISE. CW and CCW are also
	 *             permissible.
	 */
	...
	/**
	 * Intersect two segments in 3D, returns lerp params instead of actual points,
	 * because the latter can cheaply be derived from the former; more expensive the
	 * other way around. Also works for 2D.
	 * 
	 ...
	 * @return vec where vec.x is the lerp param for first segment, and vec.y is the
	 *         lerp param for the second segment
	 */
	 ...
	 * @param whatever literally pass in whatever. It is necessary because of type
	 *                 erasure in Java, which basically means Java cannot tell the
	 *                 difference between List<T> and List<List<T>> in arguments, so
	 *                 a difference in number of arguments is required for
	 *                 overloading to work
	 * @return a vector holding the average value
	 */
	...
	public class BBox {
		...
		/**
		 * Constructor that makes a bounding box from top left corner and bottom right
		 * corner
		 * 
	...
	/**
	 * Class for a bounding circle. The bounding circle is not the minimal bounding
	 * circle for now. It's just some bounding circle. It works well enough for
	 * current use cases, but we might implement minimal bounding circle in the
	 * future
	 * 
	 */
	...
	/**
	 * Find intersection between a segment and a polygon. Intersections returned are
	 * sorted from one endpoint of the segment to the other endpoint
	 * 
	 * @param p0   first endpoint of first segment
	 * @param p1   second endpoint of first segment
	 * @param poly the polygon
	 * @return a list of vectors holding the intersection points sorted from one
	...
	/**
	 * Find intersection between a segment and several polygons. Intersections
	 * returned are sorted from one endpoint of the segment to the other endpoint
	 * 
	 * @param p0   first endpoint of first segment
	 * @param p1   second endpoint of first segment
	 ...
	 * @param poly   the polygon
	 * @param trials try a couple times to be sure, otherwise we might encounter
	 *               degenerate cases
	...
	/**
	 * Check if a point is inside a polygon. Dumbed down version of
	 * pointinPolygon(3) where I pick the number of trials for you
	 * 
	 * @param p    the point in question
	 * @param poly the polygon
	 * @return true means inside, false means outside
	 ...
	 * @return either null or a point inside the polygon. If it is null, it either
	 *         means the polygon has zero or almost zero area, or that the number of
	 *         trials specified is not large enough to find one
	 */
	...
	/**
	 * Generate a random point that is inside a polygon Dumbed down version of
	 * randomPointInPolygon(2) where I pick the number of trials for you
	 * 
	 * @param poly the polygon
	 * @return either null or a point inside the polygon. If it is null, it either
	 *         means the polygon has zero or almost zero area, or that the default
	 *         number of trials is not large enough to find one
	 */
	...
	float calcPolygonTilt(ArrayList<PVector> poly) {
		...
		if (pts != null) {
			...
			if (nPoints > 2) {
				...
				// arguments: an array of points, the array's width & height, and the location
				// of the center of mass (com).
				// this function calculates the elements of a point set's tensor matrix,
				// calls the function calcEigenvector() to get the best eigenvector of this
				// matrix
				// and returns this eigenVector as a pair of doubles

				// first we look at all the pixels, determine which ones contribute mass (the
				// black ones),
...
//	  PApplet.println(centroidPoint, orientation);
	...
	float[] calcEigenvector(float[][] matrix) {
		...
		// this function takes a 2x2 matrix, and returns a pair of angles which are the
		// eigenvectors
		float A = matrix[0][0];
		float B = matrix[0][1];
		float C = matrix[1][0];
		...
		if (Q >= 0) {
			...
			// assume x1 and x2 are the elements of the eigenvector. Then, because Ax1 + Bx2
			// = lambda * x1,
			// we know that x2 = x1 * (lambda - A) / B.
			float factor2 = (PApplet.min(root1, root2) - A) / B;

			// we arbitrarily set x1 = 1.0 and compute the magnitude of the eigenVector with
	...
	/**
	 * Add a polyline to the global array of all polylines drawn Applying
	 * transformation matrices and resampling All shape drawing routines go through
	 * this function for finalization
	 * 
	 * @param poly                 a polyline
	 * @param color                the color of the polyline (0xRRGGBB)
	 * @param resampleRandomOffset whether to add a random offset during resample
	 *                             step to prevent alignment patterns
	 */
	...
	/**
	 * Simplified version for pushPolyline(3) where resampleRandomizeOffset is set
	 * to false
	 * 
	 * @param poly  a polyline
	 * @param color the color of the polyline (0xRRGGBB)
	...
	/**
	 * offset a polyline by certain amount (for outlining strokes, naive
	 * implementation)
	 * 
	 * @param poly a polyline
	...
	public float pointDistanceToLine(PVector p, PVector p0, PVector p1) {
		...
		float x2 = p1.x;
		float y2 = p1.y;
		return PApplet.abs((y2 - y1) * x0 - (x2 - x1) * y0 + x2 * y1 - y2 * x1)
				/ PApplet.sqrt(PApplet.sq(y2 - y1) + PApplet.sq(x2 - x1));
	}

	/**
	 * Calculate distance between point and a segment (when projection is not on the
	 * line, the distance becomes that to one of the endpoints)
	 * 
	 * @param p  the point in question
	 ...
	 * @return a list of polygons. When the given polygon is big at both ends and
	 *         small in the middle, the inset might break into multiple polygons
	 */
	...
	/**
	 * Offset a polygon (making it slightly smaller fitting in the original polygon,
	 * or slightly bigger wrapping the original polygon)
	 * 
	 * @param poly the polygon
...
//		    	app.image(pg,bb.x,bb.y);
	...
	/**
	 * When the general shapes of two polygons are similar, this function finds the
	 * index offset of vertices for the first polygon so that the vertices best
	 * match those of the second polygon with one-to-one correspondence. The number
	 * of vertices needs to be the same for the polygons
	 * 
	...
	/**
	 * Draw stroke (outline) for a polygon using the TANGENT style (using vector
	 * math)
	 * 
	 * @param poly the polygon
	...
	/**
	 * Draw stroke (outline) for (a) poly(gon/line)(s) using the TANGENT style
	 * (using raster algorithms)
	 * 
	 * @param polys a set of polyline/polygons, those inside another and have a
	 *              backward winding will be considered holes
	 * @param n     number of strokes
	 * @param d     spacing between the strokes
	 ...
	 * @param close whether the polyline is considered as closed (polygon) or open
	 *              (polyline)
	 * @return an array of polylines
	 */
	public ArrayList<ArrayList<PVector>> strokePolyTangentRaster(ArrayList<ArrayList<PVector>> polys, int n, float d,
			int cap, int join, boolean close) {
		BBox bb = new BBox(polys, 0);
		float scl = 1;
		if (d <= 4) {
			scl = 2;
				...
				for (int i = 0; i < polys3.size(); i++) {
					...
					if (k % 2 == 0) {
						...
						for (int j = 0; j < pp.size() + 1; j++) {
							...
							PVector p = pp.get(j % pp.size()).copy().mult(0.5f)
									.add(pp.get((j + 1) % pp.size()).copy().mult(0.5f));
							qq.add(p);
						}
						polys2.add(qq);
...
//		
	...
	/**
	 * Draw stroke (outline) for a poly(gon/line) using the PERPENDICULAR (a.k.a
	 * normal, as in "normal map", not "normal person") style (using vector math)
	 * 
	 * @param poly  the polygon
			...
			for (int i = 0; i < poly.size(); i++) {
				...
				if ((i == 0 || i == poly.size() - 1) && poly.size() > 1) {
					...
					if (i == 0) {
						...
						a = PApplet.atan2(poly.get(poly.size() - 1).y - poly.get(poly.size() - 2).y,
...
//		app.image(pg,0,0);
	...
	public ArrayList<ArrayList<PVector>> strokePolyNormalAng(ArrayList<PVector> poly, float d, float s, float ang,
			boolean close) {
//		ArrayList<ArrayList<PVector>> polys = new ArrayList<ArrayList<PVector>>();
		ArrayList<ArrayList<ArrayList<PVector>>> polyss = new ArrayList<ArrayList<ArrayList<PVector>>>();
...
//		app.image(pg,0,0);
	...
	/**
	 * draws stroke (outline) for (a) poly(gon/line)(s) using current global
	 * settings by the user. returns nothing, because it draws to the design
	 * directly.
	 * 
	 * @param polys a set of polyline/polygons, those inside another and have a
	 *              backward winding will be considered holes
	...
	public void _stroke(ArrayList<ArrayList<PVector>> polys, boolean close) {
		...
		if (STROKE_WEIGHT <= 1) {
			...
			if (STROKE_MODE == TANGENT) {
				...
				ArrayList<ArrayList<PVector>> polys2 = strokePolyTangentRaster(polys, cnt, spa, STROKE_CAP, STROKE_JOIN,
						close);
				...
				for (int i = 0; i < polys.size(); i++) {
					ArrayList<ArrayList<PVector>> polys2 = strokePolyNormal(polys.get(i), (float) STROKE_WEIGHT / 2.0f,
							STROKE_SPACING, close);
				...
				for (int i = 0; i < polys.size(); i++) {
					ArrayList<ArrayList<PVector>> polys2 = strokePolyNormalAng(polys.get(i),
							(float) STROKE_WEIGHT / 2.0f, STROKE_SPACING, STROKE_ANGLE, close);
					for (int j = 0; j < polys2.size(); j++) {
						pushPolyline(polys2.get(j), currentStroke, 0f);
					}
				}
	...
	/**
	 * hatch a polygon with CONCENTRIC (a.k.a inset) mode (using vector math). a
	 * simplified version of hatchInset(4) where orientation of polygon is always
	 * checked
	 * 
	 * @param poly    the polygon
	 * @param d       hatch spacing
	 * @param maxIter maximum number of iterations to do inset. The larger the
	 *                polygon and smaller the spacing, the more iterations is
	 *                required
	 ...
	 * @param poly             the polygon
	 * @param d                hatch spacing
	 * @param maxIter          maximum number of iterations to do inset. The larger
	 *                         the polygon and smaller the spacing, the more
	 *                         iterations is required
	 * @param checkOrientation make sure the polygon is rightly oriented, this
	 *                         should be on for user provided input, otherwise it
	 *                         might hatch outwards
	 * @return the hatching as an array of polys
	 */
	public ArrayList<ArrayList<PVector>> hatchInset(ArrayList<PVector> poly, float d, int maxIter,
	...
	/**
	 * hatch a polygon with SPIRAL mode (using vector math). a simplified version of
	 * hatchSpiral(4) where orientation of polygon is always checked
	 * 
	 * @param poly    the polygon
	 * @param d       hatch spacing
	 * @param maxIter maximum number of iterations to do inset. The larger the
	...
	public ArrayList<ArrayList<PVector>> hatchSpiral(ArrayList<PVector> poly, float d, int maxIter,
			boolean checkOrientation, boolean reverse) {
...
//				PApplet.println("???",c,p);
	...
	public ArrayList<ArrayList<PVector>> hatchSpiral_v3(ArrayList<PVector> poly, float d, int maxIter,
			boolean checkOrientation, boolean reverse) {
		ArrayList<ArrayList<PVector>> spirals = new ArrayList<ArrayList<PVector>>();
...
//			spirals.add(polys2.get(i));
	...
	public ArrayList<ArrayList<PVector>> hatchSpiral_v4(ArrayList<PVector> poly, float d, int maxIter,
			boolean checkOrientation, boolean reverse) {
		ArrayList<ArrayList<PVector>> spirals = new ArrayList<ArrayList<PVector>>();
...
//			spirals.add(polys2.get(i));
	 ...
	 * @param maxIter maximum number of iterations (i.e. seeds to begin walking
	 *                from). if the shape of polygon is weird, more seeds is needed
	 *                to reach all the corners
	 * @return the hatching as an array of polys
	 */
	public ArrayList<ArrayList<PVector>> hatchPerlin(ArrayList<PVector> poly, float d, float len, float scale,
			int maxIter) {
	 ...
	 * @return the hatching as an array of polys
	 */
	public ArrayList<ArrayList<PVector>> hatchCustomField(ArrayList<PVector> poly, VectorField vf, float d, float len,
			int maxIter) {
	 ...
	 * 
	 * @param polys a set of polygons, those inside another will be considered
	 *              holes. Holes are determined according to the even-odd rule
	 * @param ang   the angle of hatch lines
	 * @param d     hatch spacing
	 * @return the hatching as an array of polys
	 */
	...
	/**
	 * Hatch a polygon with the DRUNK style (not a particularly useful style, just
	 * to showcase how easy it is to add a new one). The vector implementation
	 * 
	 * @param poly    the polygon
	...
	public ArrayList<ArrayList<PVector>> hatchDrunkWalk(ArrayList<PVector> poly, int rad, int maxIter) {
		...
		ArrayList<ArrayList<PVector>> hatch = new ArrayList<ArrayList<PVector>>(); // this is the set of polylines
																					// representing the hatches
		hatch.add(new ArrayList<PVector>()); // typically there are >1 polylines, but in this simple example we just
												// need 1, so we add that 1 here
		...
		for (int i = 0; i < maxIter; i++) { // start fumbling
			...
			do {
				q.x = p.x + app.random(-rad, rad); // for simplicity; technically we should compute from polar
													// coordinates
				q.y = p.y + app.random(-rad, rad); // app.random() is equivalent random() in a Processing sketch, app
													// refers to the PApplet object
	...
	/**
	 * Hatch a polygon with the DRUNK style (not a particularly useful style, just
	 * to showcase how easy it is to add a new one). The raster implementation
	 * 
	 * @param poly    the polygon
	 * @param rad     max size of each drunken step in each direction
	 * @param maxIter maximum number of iterations to do the drunk walk
	...
	public ArrayList<ArrayList<PVector>> hatchDrunkWalkRaster(PImage im, int rad, int maxIter) {
		// for polygons already rendered as an image. this is usually more robust for
		// complex polygons and is the default most of the time

		ArrayList<ArrayList<PVector>> contours = PEmbroiderTrace.findContours(im); // find the polygons in the raster
																					// image
		ArrayList<ArrayList<PVector>> hatch = new ArrayList<ArrayList<PVector>>(); // this is the set of polylines
																					// representing the hatches
		...
		for (int k = 0; k < contours.size(); k++) {
			...
			for (int i = 0; i < maxIter; i++) { // start fumbling
				...
				do {
					q.x = p.x + app.random(-rad, rad); // for simplicity; technically we should compute from polar
														// coordinates
					q.y = p.y + app.random(-rad, rad); // app.random() is equivalent random() in a Processing sketch,
														// app refers to the PApplet object
				} while ((im.pixels[(int) q.y * im.width + (int) q.x] >> 16 & 0xFF) < 0x7F); // keep fumbling around
																								// until white pixel is
																								// found
				p = q;
			}
		}
	 ...
	 * @param minLen          minimum length of each segment, segment shorter than
	 *                        this will be downsampled
	 * @param maxLen          maximum length of each segment (not counting the
	 *                        randomization added on top, specified by randomize)
	 * @param randomize       amount of randomization in stitch length to avoid
	 *                        alignment patterns. 0 for none
	 * @param randomizeOffset amount of randomization to add to the offset of the
	 *                        first stitch in every polyline. 0 for none
	 * @return the resampled polyline
	 */
	public ArrayList<PVector> resample(ArrayList<PVector> poly, float minLen, float maxLen, float randomize,
			float randomizeOffset) {
...
//			}
	...
	/**
	 * Resample a polyline to make it have N vertices. Can be an upsample or a
	 * downsample. Each resultant segment will be the same length, +/- floating
	 * point percision errors
	 * 
	 * @param poly the polyline
...
//			PApplet.println("???",poly2.size(),n);
	...
	/**
	 * Resample a polyline to make it have N vertices, while keeping important
	 * turning points. Can be an upsample or a downsample. Each resultant segment
	 * have approximately same length, but compromising to the preserving of corners
	 * 
	 * @param poly the polyline
	 * @param n    the desired number of vertices
	 * @return the resampled polyline
	...
	/**
	 * Resample a set of (parallel) polylines by intersecting them with another
	 * bunch of parallel lines and using the intersections to break them up. Useful
	 * for parallel hatching.
	 * 
	 ...
	 */
	public ArrayList<ArrayList<PVector>> resampleCrossIntersection(ArrayList<ArrayList<PVector>> polys, float angle,
			float spacing, float len, float offsetFactor /* 0.5 */, float randomize) {
//		for (int i = 0; i < polys.size(); i++) {
//			for (int j = 0; j < polys.get(i).size(); j++) {
...
//			PApplet.println(x0,y0,x1,y1,px,py,qx,qy);
	...
	public ArrayList<ArrayList<PVector>> resampleCrossIntersection2(ArrayList<ArrayList<PVector>> polys1,
			ArrayList<ArrayList<PVector>> polys2, float angle1, float angle2, float spacing, float len,
			float randomize) {
		PVector mid = null;
		float dh = 0f;
...
//		return crosslines;
	 ...
	 * ellipse(), the ambigous one which can be affected by ellipseMode(). Returns
	 * nothing because the result is directly pushed to the design.
	 * 
	...
	/**
	 * Draw a rectangle using global settings. The unambigous backend for
	 * user-facing rect(), the ambigous one which can be affected by rectMode().
...
//		poly.add(new PVector(x+bl,y+h));
	...
	/**
	 * Hatch a polygon with global user settings (using vector math). Returns
	 * nothing because the result is directly pushed to the design.
	 * 
	 * @param poly the polygon
	 */
...
//			polys = hatchSpiral_v2(poly,HATCH_SPACING);
	...
	/**
	 * Hatch a raster image with global user settings. Returns nothing because the
	 * result is directly pushed to the design.
	 * 
	...
	public void hatchRaster(PImage im, float x, float y) {
		...
		if (HATCH_MODE == PARALLEL) {
			...
			if (!NO_RESAMPLE) {
				polys = resampleCrossIntersection(polys, hatch_angle, HATCH_SPACING,
						STITCH_LENGTH / getCurrentScale(hatch_angle + PApplet.HALF_PI),
						PARALLEL_RESAMPLING_OFFSET_FACTOR, RESAMPLE_NOISE);
				NO_RESAMPLE = true;
				didit = true;
			}
		} else if (HATCH_MODE == CROSS) {
			...
			polys = PEmbroiderHatchSatin.hatchSatinAngledRaster(im, hatch_angle, HATCH_SPACING,
					PApplet.max(1, (int) PApplet.ceil(STITCH_LENGTH / 2)));
		} else if (HATCH_MODE == DRUNK) {
	...
	/**
	 * Hatch a raster image with global user settings. Returns nothing because the
	 * result is directly pushed to the design. Simplified version of
	 * hatchRaster(3), draws at 0,0
	 * 
	 * @param im a processing image, PGraphics also qualify
	 */
	 ...
	 * @param p2 last point
	 * @param w  weight of the rational bezier, higher the weight, pointier the
	 *           turning
	...
	PVector rationalQuadraticBezier(PVector p0, PVector p1, PVector p2, float w, float t) {
		...
		return new PVector((PApplet.pow(1 - t, 2) * p0.x + 2 * t * (1 - t) * p1.x * w + t * t * p2.x) / u,
	...
	PVector cubicBezier(PVector p0, PVector p1, PVector p2, PVector p3, float t) {
		return p0.copy().mult(PApplet.pow(1 - t, 3)).add(p1.copy().mult(t * 3 * PApplet.pow(1 - t, 2)))
				.add(p2.copy().mult(3 * (1 - t) * t * t)).add(p3.copy().mult(t * t * t));
	}
	...
	PVector highBezier(ArrayList<PVector> P, float t) {
		...
		if (P.size() == 1) {
			...
			return highBezier(new ArrayList<PVector>(P.subList(0, P.size() - 1)), t)
					.lerp(highBezier(new ArrayList<PVector>(P.subList(1, P.size())), t), t);
		}
	}
	...
	public ArrayList<PVector> catmullRomSpline(PVector p0, PVector p1, PVector p2, PVector p3, int numberOfPoints,
			float alpha) {
...
//		    PApplet.println(C);
	 ...
	 */
	public void quad(float x0, float y0, float x1, float y1, float x2, float y2, float x3, float y3) {
		ArrayList<PVector> poly = new ArrayList<PVector>();
		poly.add(new PVector(x0, y0));
		poly.add(new PVector(x1, y1));
	...
	/**
	 * Add a cubic bezier vertex to the current polygon/polyline. This must be
	 * preceded by beginShape() and at least a vertex() An alias of cubicVertex
	 * 
	...
	/**
	 * Add a rational quadratic bezier vertex to the current polygon/polyline. This
	 * must be preceded by beginShape() and at least a vertex()
	 * 
	 * @param x1 x coordinate of first control point
	 * @param y1 y coordinate of first control point
	 * @param x2 x coordinate of second control point
	...
	/**
	 * Add a quadratic bezier vertex to the current polygon/polyline. This must be
	 * preceded by beginShape() and at least a vertex()
	 * 
	 * @param x1 x coordinate of first control point
	 * @param y1 y coordinate of first control point
	 * @param x2 x coordinate of second control point
	...
	/**
	 * Add a quartic bezier vertex to the current polygon/polyline. This must be
	 * preceded by beginShape() and at least a vertex()
	 * 
	 * @param x1 x coordinate of first control point
	 * @param y1 y coordinate of first control point
	...
	/**
	 * Add a quintic bezier vertex to the current polygon/polyline. This must be
	 * preceded by beginShape() and at least a vertex()
	 * 
	 * @param x1 x coordinate of first control point
	 * @param y1 y coordinate of first control point
	 ...
	 */
	public void quinticVertex(float x1, float y1, float x2, float y2, float x3, float y3, float x4, float y4, float x5,
			float y5) {
		ArrayList<PVector> poly = new ArrayList<PVector>();
		poly.add(new PVector(x1, y1));
		poly.add(new PVector(x2, y2));
	...
	/**
	 * Add a higher-order bezier vertex to the current polygon/polyline. This must
	 * be preceded by beginShape() and at least a vertex()
	 * 
	...
	/**
	 * End drawing a polygon. at this moment the polygon will be actually drawn to
	 * the design
	 * 
...
//					HATCH_MODE = CROSS;
					...
					if (HATCH_MODE == CROSS) {
						if (EXPERIMENTAL_CROSS_RESAMPLE && !NO_RESAMPLE) {
							...
							ArrayList<ArrayList<PVector>> polys2 = hatchParallelComplex(polyBuff, hatch_angle2,
									HATCH_SPACING);
							polys = resampleCrossIntersection2(polys, polys2, hatch_angle, hatch_angle2, HATCH_SPACING,
									STITCH_LENGTH / getCurrentScale(hatch_angle + PApplet.HALF_PI), RESAMPLE_NOISE);
...
//					app.image(pg,0,0);
	...
	/**
	 * Begin a contour within the current polygon, if this contour has negative
	 * winding and is inside the current polygon or another contour defined between
	 * beginShape() and endShape() that is not a hole, this will be a hole
	 */
	 ...
	 * 
	 * @param color    whether to visualize color, if false, will use random colors;
	 *                 if stitches argument is true, this will have no effect and
	 *                 black will always be used for visibility
	 * @param stitches whether to visualize stitches, i.e. little dots on end of
	 *                 segments
	 * @param route    whether to visualize the path between polylines that will be
	 *                 taken by embroidery machine/plotter. To be able to not see a
	 *                 mess when enabling this option, try optimize()
	 */
...
//					PApplet.println(p0,p1);
	...
	/**
	 * Visualize the current design on the main Processing canvas, using default set
	 * of options
	 */
	...
	/**
	 * Supposed to initialize something, but as we currently don't need to, this is
	 * a NOP
	 */
	...
	/**
	 * Double the number of vertices in the polyline by spliting every segment by
	 * half
	 * 
	 * @param poly the polyline
	 * @return the resampled polyline
	...
	/**
	 * Approximately halfing the number of vertices in the polyline but preserves
	...
	public void endCull() {
			...
			while (j < groups.get(i).size()) {
				...
				for (int k = 0; k < groups.get(i).get(j).size(); k++) {
					...
					if (!ok) {
						...
						ArrayList<PVector> rhs = new ArrayList<PVector>(
								groups.get(i).get(j).subList(k + 1, groups.get(i).get(j).size()));

						ArrayList<PVector> s0 = new ArrayList<PVector>(
								groups.get(i).get(j).subList(k, PApplet.min(k + 1, groups.get(i).get(j).size())));
						ArrayList<PVector> s1 = new ArrayList<PVector>(
								groups.get(i).get(j).subList(PApplet.max(k - 3, 0), k + 1));
		...
		for (int i = 0; i < polylines2.size(); i++) {
			polylines2.set(i, resampleHalfKeepCorners(resampleHalfKeepCorners(polylines2.get(i), PConstants.PI * 0.1f),
					PConstants.PI * 0.1f));
		}
...
//		app.image(pg,0,0);
	 ...
	 * @param minVertices strokes with too few segments (those tiny ones) will be
	 *                    discarded
	 * @param maxVertices maximum number of vertices for each stroke
	 * @param maxIter     maximum number of iterations (i.e. seeds to begin walking
	 *                    from). if the shape of polygon is weird, more seeds is
	 *                    needed to reach all the corners
	 */
	public ArrayList<ArrayList<PVector>> perlinField(PImage mask, float d, float perlinScale /* 0.01f */,
			float deltaX /* 20 */, int minVertices /* 2 */, int maxVertices /* 100 */, int maxIter) {
	 ...
	 *                    needed to reach all the corners
	 */
	public ArrayList<ArrayList<PVector>> customField(PImage mask, VectorField vf, float d, int minVertices,
			int maxVertices, int maxIter) {
		ArrayList<ArrayList<PVector>> polys = new ArrayList<ArrayList<PVector>>();
	...
	/**
	 * Lerp a number around 360 degrees, meaning lerp360(358,2) gives 0 instead of
	 * 180 Useful for lerping hues and angles
	 * 
	...
	public float lerp360(float h0, float h1, float t) {
		float[][] methods = new float[][] { { Math.abs(h1 - h0), PApplet.map(t, 0, 1, h0, h1) },
...
//        		PApplet.println(h0,h1,i);
	...
	/**
	 * Computes the accurate distance transform by actually mesuring distances
	 * (slow). See PEmbroiderTrace for the fast approximation
	 * 
		...
		while (j < polys.size()) {
			...
			for (int k = 0; k < polys.get(j).size(); k++) {
				...
				if (!ok) {
					...
					ArrayList<PVector> s0 = new ArrayList<PVector>(
							polys.get(j).subList(k, PApplet.min(k + 1, polys.get(j).size())));
	...
	/**
	 * Compute isolines for a grayscale image using findContours on multiple
	 * thresholds.
	 * 
...
//				polys.add(resampleHalfKeepCorners(resampleHalf(isos.get(i).get(j)),0.1f));
				...
				if (CONCENTRIC_ANTIALIGN > 0) {
					...
					if (i % 2 == 0) {
						...
						for (int k = 0; k < pp.size() + 1; k++) {
							...
							PVector p = pp.get(k % pp.size()).copy().mult(0.5f)
									.add(pp.get((k + 1) % pp.size()).copy().mult(0.5f));
							qq.add(p);
						}
						polys.add(qq);
	...
	/**
	 * Run TSP on polylines in the current design to optimize for connective path
	 * length
	 * 
	 * @param trials  number of trials. the more times you try, the higher chance
	...
	/**
	 * Simplified version of optimize(2) where the trial and maxIter is picked for
	 * you
	 */
	...
	/**
	 * Begin a block of stitch order optimization (TSP solver), to be paired with
	 * endOptimize()
	 * 
	 * @param reorderColor number of seconds to try to reorder color to reduce
	 *                     number of color changes without modifying the final look
	 *                     of the design, 0 means no color reordering
	...
	public void endOptimize() {
		...
		if (optColor > 0) {
			...
			ArrayList<ArrayList<PVector>> p = new ArrayList<ArrayList<PVector>>(
					polylines.subList(optBlockIdx0, polylines.size()));
...
//			PApplet.println("!!!",optBlockIdx0,polylines.size(),p.size(),c.size());
	...
	/**
	 * Change font of text. Notice this function is overloaded with one version for
	 * PFont and one for hershey font, with identical API This one is for the
	 * hershey font
	 * 
	...
	/**
	 * Change font of text. Notice this function is overloaded with one version for
	 * PFont and one for hershey font, with identical API This one is for PFont
	 * 
...
//			 b.x += app.random(10);
			...
			float m = polylines.get(i).get(polylines.get(i).size() - 1)
					.dist(polylines.get(i).get(polylines.get(i).size() - 2));
			float s = x / m;
			PVector d = polylines.get(i).get(polylines.get(i).size() - 1).copy().mult(1 - s)
					.add(polylines.get(i).get(polylines.get(i).size() - 2).copy().mult(s));
...
//			 d.x += app.random(10);
	...
	ArrayList<Integer> reorderColorMonteCarlo(ArrayList<ArrayList<PVector>> polys, ArrayList<Integer> cols, float d,
			float wait) {
...
//				standard.drawRender().save("/Users/studio/Downloads/rcmc-" + hashCode() + ".png");
				...
				for (int i = 0; i < Integer.MAX_VALUE; i++) {
					...
					if (sol.score < solutions.get(solutions.size() - 1).score) {
						PApplet.println(
								logPrefix + " Color opt trial # " + i + ", improved color changes to: " + sol.score);
						solutions.add(sol);
					}
					if (i % 100 == 0) {
...
//			throw new RuntimeException();
			...
			int op = javax.swing.JOptionPane.showOptionDialog(null,
					"Some stitches are out of bounds! Please select an option:\n(use setOutOfBoundsHandler to silence this popup)",
					"Out of Bounds Warning", javax.swing.JOptionPane.DEFAULT_OPTION,
